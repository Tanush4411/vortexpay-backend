package com.chrizlove.vortexpay.common_lib.idempotency;

import com.chrizlove.vortexpay.common_lib.context.MerchantContext;
import com.chrizlove.vortexpay.common_lib.exceptions.IdempotencyConflictException;
...
//        System.out.print(request.getRequestURI()+" notice this!!!");
//        log.info("notice this!!!");
//        if(!GUARDED_METHODS.contains(request.getMethod())) {
...
//
//        String rawKey=request.getHeader("X-Idempotency-Key");
//        if(rawKey==null || rawKey.isBlank()) {      //no-idempotency key
//            filterChain.doFilter(request,response);
//            return;
//        }
//
...
//
//            if(existing.isPresent() && !IdempotencyStore.IN_PROGRESS.equals(existing.get())) {
//                //its not in progress, but coming from the actual value stored in redis
//                replay(request, response,existing.get());
//            }else{
//                //its already in progress by another thread
...
//                String stored=status+SEPARATOR+body;
//                idempotencyStore.store(key,stored,COMPLETED_TTL);
//                log.warn("IdempotencyFilter: stored response status={} key={}", status, key);
//            }
//            else{
//                //Error or empty - delete placeholder so client can retry cleanly
//                idempotencyStore.delete(key);
...
//
//            //Always flush buffered body to the actual response
//            //If this is skipped the client receives an empty box
//            wrapper.copyBodyToResponse();
//        }
//
//    }
