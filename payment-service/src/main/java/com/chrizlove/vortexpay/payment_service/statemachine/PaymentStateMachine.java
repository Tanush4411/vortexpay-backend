...
import com.chrizlove.vortexpay.common_lib.exceptions.InvalidStateTransitionException;
...
public class PaymentStateMachine {
    ...
    private static final Map<Transition, PaymentStatus> TRANSITION = Map.ofEntries(
            ...
            Map.entry(new Transition(PaymentStatus.AUTHORIZING,PaymentEvent.CAPTURE_TIMEOUT),PaymentStatus.AUTH_EXPIRED),
    ...
    public PaymentStatus transition(PaymentStatus current, PaymentEvent event){
        ...
        if(next==null) throw new InvalidStateTransitionException(current.name(),event.name());
        return next;
    }
}
